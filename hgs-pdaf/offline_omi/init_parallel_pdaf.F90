...
! !USES:
  USE mod_parallel_pdaf, &
       ...
       local_npes_model, task_id, MPI_UNDEFINED, COMM_couple, &
       mype_world, npes_world, MPI_COMM_WORLD, mype_model, npes_model, &
       COMM_model, MPIerr, abort_parallel
  USE mod_assim_pdaf, &
       ONLY: dim_ens
...
!EOP
  ...
  IF (mype_world == 0) &
       WRITE (*, '(/1x, a)') 'HGS-PDAF: Initialize communicators for assimilation with PDAF'

  IF (mype_world == 0) THEN
     WRITE (*,'(a,3x,a,i6)') 'HGS-PDAF', 'model tasks: ', n_modeltasks
  END IF
  ...
  IF (n_modeltasks /= npes_world) THEN
     WRITE (*,*) 'ERROR: Inconsistent setting of processes for ensmeble integrations!'
     call abort_parallel()
  END IF
  ...
  END DO
  !local_npes_model = 1 ! each model run uses only 1 proc
  ...
  ! *** (Split COMM_ENSEMBLE)                 ***
  ! This part is needed only when local_npes_model /= 1
  pe_index = 0
  ...
  if (screen > 1) then
    write (*,'(a,i5,a,i5,a,i5,a,i5)') 'HGS-PDAF: mype(w)= ', mype_world, &
...
!  IF (filterpe) THEN
 ...
 !    my_color = MPI_UNDEFINED
  ...
  IF (screen > 0) THEN
     ...
     IF (task_id > 1) THEN
        WRITE (*,'(2x, i4, 4x, i4, 4x, i3, 4x, i3, 4x, i3, 4x, i3, 5x, l3)') &
         mype_world, mype_filter, task_id, mype_model, color_couple, mype_couple, filterpe
     END IF
     CALL MPI_Barrier(MPI_COMM_WORLD, MPIerr)
